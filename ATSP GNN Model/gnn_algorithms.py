...
— adaptados de "GNN Model/gnn_algorithms.py" (originalmente de
https://github.com/walidgeuttala/atsp, src/algorithms.py).

Mudanças para o ATSP:
    * Os operadores vêm de `atsp_operators`: o 2-opt (inválido para matrizes
      assimétricas, pois reverte segmentos) foi trocado pelo Or-opt.
    * Todo o resto já era direcionalmente correto: `tour_cost` soma arcos no
      sentido do tour, `nearest_neighbor` usa G.neighbors (= sucessores no
      DiGraph), `nx.attr_matrix` devolve a matriz assimétrica e as penalidades
      do GLS usam acesso direcionado G[u][v].
"""
...
import atsp_operators as operators
...
def local_search(init_tour, init_cost, D, first_improvement=False):
    ...
    while improved and cnt < 100:
        ...
        improved = False
        for operator in [operators.or_opt_a2a, operators.relocate_a2a]:
        ...
        while moves < perturbation_moves:
            ...
            for n in max_util_e:
                if n != 0:  # not the depot
                    ...
                    for operator in [operators.or_opt_o2a, operators.relocate_o2a]:
                        moved = False
