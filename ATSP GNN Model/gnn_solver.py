"""
Solver GNN ATSP para o comparativo — adaptado de "GNN Model/gnn_solver.py"
(inferência de https://github.com/walidgeuttala/atsp, src/test.py).

Pipeline por instância:
    1. Constrói o line graph DIRECIONADO da instância (cacheado por tamanho) e
       escala os pesos dos arcos com os scalers do treino;
    2. O GNN prediz o regret de cada arco (regret_pred, truncado em >= 0);
    3. Tour inicial: Nearest-Neighbor guiado por regret_pred;
    4. Melhoria: Guided Local Search (guia = regret_pred, operadores Or-opt +
       relocate, válidos para matrizes assimétricas) até o limite de tempo.

Requer os checkpoints treinados por train_gnn.py em checkpoints/atsp{n}/.
"""
...
def _load_bundle(n, checkpoint_dir):
    ...
    ckpt_dir = os.path.join(checkpoint_dir, f"atsp{n}")
    params_path = os.path.join(ckpt_dir, "params.json")
    if not os.path.exists(params_path):
        raise FileNotFoundError(
            f"Checkpoint do GNN ATSP para n={n} não encontrado em '{ckpt_dir}'. "
            f"Treine antes com: python \"ATSP GNN Model/train_gnn.py\" --sizes {n}"
        )
...
def _line_graph(n):
    """Line graph direcionado do K_n completo (idêntico para qualquer instância de tamanho n)."""
    if n in _LG_CACHE:
        return _LG_CACHE[n]

    G = nx.DiGraph()
    G.add_nodes_from(range(n))
    G.add_edges_from(itertools.permutations(range(n), 2))
    lG = nx.line_graph(G)
...
def solve_gnn_atsp(distance_matrix, seed, time_limit=None, perturbation_moves=20,
                   checkpoint_dir=DEFAULT_CHECKPOINT_DIR):
    """Resolve o ATSP com GNN (regret_pred) + Guided Local Search.
    ...
    # 1-2. Predição do regret de cada arco
