...
homogêneo por tamanho, como no repositório original) e salva em:
    ...
    checkpoints/tsp{n}/params.json              (hiperparâmetros do modelo)
    checkpoints/tsp{n}/scalers.pkl              (cópia dos scalers do dataset)

Diferenças em relação ao train.py original (justificadas para CPU/Windows):
    * device padrão = cpu (a máquina alvo não tem CUDA);
    * hidden_dim/num_layers menores por padrão (config de teste do próprio
      repositório: hidden 64) — treino roda em CPU em tempo viável;
    * sem TensorBoard/torch.jit (não são usados na inferência do comparativo);
    * avaliação final: gap do tour Nearest-Neighbor guiado por regret_pred
      em relação ao custo ótimo, sobre o conjunto de validação.
...
from gnn_dataset import TSPDataset
from gnn_algorithms import nearest_neighbor, tour_cost

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

# Batch (nº de grafos por batch) por tamanho — batch pequeno (como o batch 10
# do repositório original) garante passos de otimização suficientes por época
# com poucas instâncias; grafos maiores têm line graphs com muito mais arestas,
# então o batch diminui ainda mais para caber em CPU/RAM.
BATCH_SIZES = {5: 8, 10: 8, 20: 8, 50: 4, 100: 2}

# Épocas por tamanho: os pequenos são baratos (segundos/época) e precisam de
# mais épocas para acumular passos de gradiente; n=100 custa ~100s/época.
...
def train_size(n, args):
    data_dir = os.path.join(args.data_dir, f"tsp{n}")
    ckpt_dir = os.path.join(args.checkpoint_dir, f"tsp{n}")
    os.makedirs(ckpt_dir, exist_ok=True)

    train_data = TSPDataset(os.path.join(data_dir, "train.txt"))
    val_data = TSPDataset(os.path.join(data_dir, "val.txt"))
    ...
    for epoch in range(n_epochs):
        ...
        print(f"[tsp{n}] época {epoch + 1:3d}/{n_epochs} | "
        ...
        if counter >= args.patience:
            print(f"[tsp{n}] early stopping (paciência {args.patience})", flush=True)
            break
    ...
    print(f"[tsp{n}] melhor val loss {best_score:.5f} (época {checkpoint['epoch'] + 1}) | "
          f"gap NN(regret_pred) na validação: {gap:.2f}%", flush=True)


def parse_args():
    parser = argparse.ArgumentParser("Treino do GNN (regret) por tamanho de instância")
