# =============================================================================
# Setup para Google Colab — Instalação de dependências e compilação do LKH
# =============================================================================
...
# 1. Instalar pacotes Python necessários
subprocess.run([sys.executable, "-m", "pip", "install", "-q",
                "ortools", "numpy", "pandas", "openpyxl"], check=True)

# 2. Baixar, extrair e compilar o LKH-3 (uma vez por sessão do Colab)
LKH_DIR = "/content/LKH-3.0.13"
LKH_BIN = os.path.join(LKH_DIR, "LKH")

if not os.path.exists(LKH_BIN):
    subprocess.run(["wget", "-q", "http://akira.ruc.dk/~keld/research/LKH-3/LKH-3.0.13.tgz",
                    "-O", "/content/LKH-3.0.13.tgz"], check=True)
    subprocess.run(["tar", "-xzf", "/content/LKH-3.0.13.tgz", "-C", "/content/"], check=True)
    subprocess.run(["make", "-C", LKH_DIR, "-j"], check=True)

# =============================================================================
# Código do TSP50 (lógica inalterada — apenas o caminho do LKH foi adaptado)
# =============================================================================
...
def solve_lkh3(distance_matrix, seed):
    ...
    # Define unique filenames based on the seed
    tsp_filename = f"temp_seed_{seed}.tsp"
    par_filename = f"temp_seed_{seed}.par"
    tour_filename = f"temp_seed_{seed}.tour"

    # A. Write the TSP file (Explicit Full Matrix format)
    with open(tsp_filename, "w") as f:
        f.write(f"NAME : temp_seed_{seed}\n")
        f.write("TYPE : TSP\n")
    ...
    # C. Run LKH binary (compiled in /content/LKH-3.0.13/LKH)
    lkh_path = LKH_BIN
...
# -------------------------------------------------------------------------
def main():
    print("Iniciando o comparativo de algoritmos para o TSP...")
    print(f"Configurações: Paradas (N) = {NUM_NODES}, Seeds = {NUM_SEEDS}\n")

    results = []

    for seed in range(1, NUM_SEEDS + 1):
        print(f"--- Processando Seed {seed:02d} ---")
        ...
        print(f"  [Simplex]        Custo: {simplex_cost:<5d} | Tempo: {simplex_time:.4f}s")
        ...
        print(f"  [LKH3]           Custo: {lkh_cost:<5d} | Tempo: {lkh_time:.4f}s")
        ...
        print(f"  [OR-Tools]       Custo: {ortools_cost:<5d} | Tempo: {ortools_time:.4f}s")
        ...
        print(f"  [NearestNeighbor]Custo: {nn_cost:<5d} | Tempo: {nn_time:.4f}s")
        ...
        print(f"  [ACO - ACS]      Custo: {aco_cost:<5d} | Tempo: {aco_time:.4f}s")
    ...
    # Salvar no diretório /content do Colab e disparar download automático
    output_filename = "/content/Comparação TSP50.xlsx"
    ...
    try:
        ...
        print(f"(Download automático indisponível: {e})")
